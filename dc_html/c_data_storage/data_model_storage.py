...
import logging
import numpy as np
from c_data_storage.interfaces import IDataStorage

class DataModelStorage(IDataStorage):
    """
    ...
    Follows Single Responsibility Principle by focusing only on data storage operations.
    """
    ...
    def initialize(self, scan_index: List[int], sensor: str, stream: str) -> None:
        ...
        Args:
            ...
            sensor: Sensor name for logging purposes
            stream: Stream name for logging purposes
        """
    ...
    def _process_dataset(self, dataset: Any, key_stream: str, signal_name: str, key_grp: str) -> None:
    ...
    # Getters for accessing internal state (needed for formatting)
    def get_data_container(self) -> Dict[str, List]:
        """Get the data container."""
        return self._data_container
    
    def get_signal_to_value(self) -> Dict[str, Any]:
        """Get the signal to value mapping."""
        return self._signal_to_value
    
    def get_value_to_signal(self) -> Dict[str, str]:
        """Get the value to signal mapping."""
        return self._value_to_signal
    
    def get_parent_counter(self) -> int:
        """Get the parent counter."""
        return self._parent_counter
    
    def get_child_counter(self) -> int:
        """Get the child counter."""
        return self._child_counter
    
    def get_stream_name(self) -> str:
        """Get the stream name."""
        return getattr(self, 'stream_name', 'N/A')

    @staticmethod
    def round_to_2_decimals(data: Any) -> tuple:
