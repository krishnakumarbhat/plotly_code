        ...
        for index, (input_file, output_file) in enumerate(self.address_map.items()):
            for sensor in sensor_list:
                ...
                with h5py.File(input_file, 'r') as hdf_file:
                    ...
                    self.data_container_in = {j: np.array([]) for j in scan_index}
                    data_group =  hdf_file[f"data/{sensor}/"]
                    self.data_container_in ,self.val_sig_map_in , self.sig_val_map_in = hdf_parser.HDF5Parser.parse(data_group, self.data_container_in, self.val_sig_map_in ,self.sig_val_map_in ,scan_index)
                ...
                with h5py.File(output_file, 'r') as hdf_file_out:
                    ...
                    self.data_container_out = {j: np.array([]) for j in scan_index}
                    # self.address_map.value() = self.address_map.value() + "_" + sensor
                    self.data_container_out, self.val_sig_map_out , self.sig_val_map_in = hdf_parser.HDF5Parser.parse(data_group, self.data_container_out, self.val_sig_map_out,self.sig_val_map_in ,scan_index)

                print(f"Output Data: {self.data_container_out}, Value Signal Map Out: {self.val_sig_map_out}")
                DataContainer( self.data_container_in, self.val_sig_map_in,self.sig_val_map_out, self.data_container_out, self.val_sig_map_out,self.sig_val_map_out, self.html_name)
