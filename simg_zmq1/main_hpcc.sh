...
elif [[ -d "$ROOT_DIR/main_html" || -d "$ROOT_DIR/KPI" || -d "$ROOT_DIR/rag" ]]; then
...
ui_cmd=(
    ...
    --env "DATABASE_URL=sqlite:///$MAIN_HTML_CACHE_DIR/hpc_tools_dev.db"
...
if [[ "$HPCC_AUTO_START_RAG" =~ ^(1|true|yes|y)$ ]] && [[ -x "$SCRIPT_DIR/rag/run_rag.sh" ]]; then
