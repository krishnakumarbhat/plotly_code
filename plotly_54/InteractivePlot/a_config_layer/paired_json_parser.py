            ...
            if len(inputs) == len(outputs):
                ...
                for i, (input_pair, output_pair) in enumerate(zip(inputs, outputs)):
                    if len(input_pair) == 2 and len(output_pair) == 2:
                        ...
                        print(f"Warning: Pair #{i+1} has incorrect number of files (should be 2 files per pair)")
            else:
                print(f"Input and output pair counts do not match: {len(inputs)} != {len(outputs)}")
