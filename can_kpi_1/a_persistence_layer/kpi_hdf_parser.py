    ...
    def parse_file(self, hdf_path: str) -> Dict[str, Any]:
        ...
        for sensor_id, sensor_data in raw.items():
            ...
            if len(scan_idx) == 0 and not det_signals:
                logger.info(
                    f"Sensor {sensor_id}: no scan index and no detections — skipping"
                )
                continue

            if not det_signals:
                logger.info(
                    f"Sensor {sensor_id}: no detection signals (keeping header/alignment if present)"
                )
            ...
            parsed[sensor_id] = {
                ...
                "alignment_signals": self._reader.extract_alignment_signals(
                    sensor_data
                ),
