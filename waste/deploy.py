...
# Cluster defaults
DEFAULT_HOST = "10.214.45.45"
DEFAULT_BASE_DEST = "/net/8k3/e0fs01/irods/PLKRA-PROJECTS/RNA-SDV-SRR7/2-Sim/USER_DATA"
...
def build_simg() -> bool:
    ...
    if is_windows():
        ...
        wsl_project_path = str(PROJECT_ROOT).replace("C:", "/mnt/c").replace("\\", "/")
        wsl_script = f"{wsl_project_path}/scripts/wsl_build_simg.sh"
        
        print(f"Running in WSL: {wsl_script} {IMAGE_NAME}")
        result = subprocess.run(
            ["wsl", "bash", "-c", f"cd '{wsl_project_path}' && ./scripts/wsl_build_simg.sh {IMAGE_NAME}"],
            cwd=str(PROJECT_ROOT)
        )
        return result.returncode == 0
    else:
        # On Linux, run directly
        script_path = PROJECT_ROOT / "scripts" / "wsl_build_simg.sh"
        if script_path.exists():
            result = run_cmd(["bash", str(script_path), IMAGE_NAME], cwd=str(PROJECT_ROOT), check=False)
            return result.returncode == 0
        else:
            # Direct apptainer/singularity build
            for tool in ["apptainer", "singularity"]:
                if subprocess.run(["which", tool], capture_output=True).returncode == 0:
                    result = run_cmd(["sudo", tool, "build", IMAGE_NAME, "Singularity.def"], 
...
def sync_to_cluster(host: str, user: str, password: str, dest: str) -> bool:
    """Sync project files to cluster using sync_to_cluster.py."""
    print("\n" + "=" * 60)
    print("Syncing files to cluster")
    print("=" * 60)
    
    sync_script = PROJECT_ROOT / "sync_to_cluster.py"
...
def copy_simg_to_cluster(host: str, user: str, password: str, dest: str) -> bool:
    """Copy the built .simg file to cluster via scp."""
    print("\n" + "=" * 60)
    print("Copying container image to cluster")
    print("=" * 60)
    
    local_image = PROJECT_ROOT / IMAGE_NAME
    if not local_image.exists():
        print(f"ERROR: Image not found: {local_image}")
        return False
    
    remote_path = f"{user}@{host}:{dest}/{IMAGE_NAME}"
    print(f"Copying {local_image} -> {remote_path}")
    
    # Use sshpass if available, otherwise plain scp (will prompt)
    try:
...
def run_on_cluster(host: str, user: str, password: str, dest: str, port: int = 5001) -> bool:
    """SSH to cluster and run the container."""
    print("\n" + "=" * 60)
    print("Starting container on cluster")
    print("=" * 60)
    
    run_cmd_remote = f"""
cd {dest}
chmod +x run_all_services_simg.sh 2>/dev/null || true
export DATABASE_URL="sqlite:///{dest}/.cache_html/hpc_tools.db"
export PORT={port}
nohup ./run_all_services_simg.sh ./{IMAGE_NAME} {port} > server.log 2>&1 &
...
"""
    ...
    try:
        ...
        print(f"  cd {dest}")
        print(f"  ./run_all_services_simg.sh ./{IMAGE_NAME} {port}")
        return False
...
def main():
    ...
    parser.add_argument("--host", default=os.environ.get("CLUSTER_HOST", DEFAULT_HOST))
    ...
    # Default destination
    if not args.dest:
        args.dest = f"{DEFAULT_BASE_DEST}/{args.user}/all_services"
    ...
    if args.sync and success:
        if not sync_to_cluster(args.host, args.user, password, args.dest):
            print("\nSync failed!")
            success = False
    
    if args.copy_image and success:
        if not copy_simg_to_cluster(args.host, args.user, password, args.dest):
            print("\nImage copy failed!")
            success = False
    
    if args.run and success:
        if not run_on_cluster(args.host, args.user, password, args.dest, args.port):
            print("\nRun failed!")
            success = False
    ...
    if success:
        ...
        if args.run:
            print(f"\nAccess the app at: http://{args.host}:{args.port}/html")
    else:
