...
DEFAULT_HOST = "10.214.45.45"  # Krakow
...
def main():
    ...
    parser.add_argument("--host", default=DEFAULT_HOST, help=f"Remote host (default: {DEFAULT_HOST})")
    parser.add_argument("--user", default=os.environ.get("USERNAME", getpass.getuser()), help="SSH username")
    parser.add_argument("--dest", default=None, help="Remote destination path (default: auto from user)")
    ...
    # Determine local base (this script's parent directory)
    ...
    if args.dest:
        ...
        remote_base = f"{DEFAULT_BASE_DEST}/{args.user}/all_services"
    
    print("=" * 60)
    print("all_services Cluster Sync")
    print("=" * 60)
    print(f"Local:  {local_base}")
    print(f"Remote: {args.user}@{args.host}:{remote_base}")
    print("=" * 60)
