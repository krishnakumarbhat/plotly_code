...
<body>
  ...
  <script>
    ...
    // Detect if running via server (http) or file protocol
    const isHttpMode = window.location.protocol.startsWith('http');
    const API_BASE = isHttpMode ? window.location.origin : null;

    async function checkServerStatus() {
      const statusEl = document.getElementById('serverStatus');
      if (!isHttpMode) {
        statusEl.textContent = 'Mode: File (read-only save)';
      ...
      try {
        const resp = await fetch(API_BASE + '/api/read?path=.', { method: 'GET' });
        if (resp.ok || resp.status === 404) {
          serverAvailable = true;
          statusEl.textContent = 'Server: Online (save enabled)';
        ...
        serverAvailable = false;
        statusEl.textContent = 'Server: Offline (download mode)';
        statusEl.className = 'server-status server-offline';
      }
    }
    ...
    async function loadExistingComment(entry) {
      ...
      // Try to load from server if available, otherwise use embedded content
      if (serverAvailable && API_BASE) {
        try {
          const resp = await fetch(API_BASE + '/api/read?path=' + encodeURIComponent(path));
          if (resp.ok) {
            const data = await resp.json();
            commentBox.value = data.content || '';
            return;
          }
        } catch (e) {
          console.warn('Failed to load from server, using embedded content:', e);
        }
      }
    ...
    async function saveComment(entry) {
      ...
      // If server is available, save directly
      if (serverAvailable && API_BASE) {
        ...
        try {
          ...
          if (resp.ok) {
            ...
            // Update embedded content too
            entry.comment_content = content;
      ...
      const message = 'Server not running. Save will download the file.\\n\\n' +
                      'After download, move file to:\\n' + absPath + '\\n\\n' +
                      'File name:';
      ...
      alert('File downloaded!\\n\\nTo overwrite the original, copy/move to:\\n\\n' + absPath);
    ...
    def run(self) -> None:
        ...
        if not matched:
            ...
            for k in matched:
                ...
                html_folder = this._get_html_folder_name(k)
                ...
                console.log(f"  {k} -> {len(html_files)} HTML file(s), video: {video_url}");
                ...
                if text_info {
                    ...
                    comment_abs_path = os.path.abspath(os.path.join(this.output_dir, comment_path)).replace("\\", "/");
                } else {
                    base_name, _ext = os.path.splitext(video_name);
                    comment_path = `db/video/log_txt/${base_name}.txt`;
                    comment_content = "";
                    comment_abs_path = os.path.abspath(os.path.join(this.output_dir, comment_path)).replace("\\", "/");
                }
                mapping[k] = {
        ...
        self._build_html(this.output_html, mapping);
        print(f"Viewer HTML generated at: {this.output_html}");
    }


def main() -> None:
    if len(sys.argv) < 3:
        print("Usage: python main_cloud.py <html_root> <video_root> [output_html] [--serve [port]]", file=sys.stderr)
        sys.exit(1)
    ...
    while i < len(sys.argv):
        ...
        if arg == "--serve":
            ...
            if i + 1 < len(sys.argv) && sys.argv[i + 1].isdigit() {
    ...
    if (serve) {
        try {
            ...
            run_server(app.output_dir, port);
        } catch (ImportError) {
            print("Error: Flask not installed. Run: pip install flask flask-cors", file=sys.stderr);
            sys.exit(1);
        }
    }
}
