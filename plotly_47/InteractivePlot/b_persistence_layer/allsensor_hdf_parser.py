...
class AllsensorHdfParser(PersensorHdfParser):
    ...
    def _process_sensor_stream(self, args: Tuple) -> Optional[DataPrep]:
        """
        Process a single sensor/stream combination.
        
        Args:
            args: Tuple containing (input_file, output_file, sensor, stream, html_name)
            
        Returns:
            Optional[DataPrep]: DataPrep object if processing was successful
        """
        input_file, output_file, sensor, stream, html_name = args
        
        # Create new storage instances for this sensor/stream
        ...
        # Process input file
        self._process_file(input_file, sensor, stream, input_data)
        
        # Process output file
        self._process_file(output_file, sensor, stream, output_data)
    ...
    def _process_file(self, file_path: str, sensor: str, stream: str, data_storage: DataModelStorage) -> None:
        """
    ...
    def parse(self) -> List[DataPrep]:
        ...
        for input_file, output_file in self.address_map.items():
            ...
            # Sequential processing to avoid multiprocessing serialization issues
