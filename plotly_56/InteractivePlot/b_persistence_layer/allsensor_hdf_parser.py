        ...
        for input_file, output_file in self.address_map.items():
            ...
            for sensor in sensor_list:
                ...
                for stream in streams:
                    html_name = f"{base_name}_{sensor}_{stream}.html"
                    if stream in Gen7V1_v2_streams:
                        ...
                        if sensor_stream_path in hdf_file_in:
                            ...
                            header_variants = ['Stream_Hdr', 'stream_hdr', 'StreamHdr', 'STREAM_HDR', 'streamheader', 'stream_header','HEADER_STREAM']
                            ...
                            if header_path:
                                ...
                                a= time.time()
                                input_data = hdf_parser.HDF5Parser.parse(data_group, input_data, scan_index,header_variants)
                                b = time.time()
                                print(f"time taken by input parsing of {b-a} in {stream}")
                        ...
                        if sensor_stream_path in hdf_file_out:
                            ...
                            if header_path:
                                ...
                                a= time.time()
                                output_data = hdf_parser.HDF5Parser.parse(data_group, output_data, scan_index,header_variants)
                                b = time.time()
                                print(f"time taken by output parsing of {b-a} in {stream}")
                        ...
                        if input_data._data_container or output_data._data_container:
                            ...
                            b= time.time()
                            print(f"time taken by dataprep parsing of {b-a} in {stream}")
