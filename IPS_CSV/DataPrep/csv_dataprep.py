...
class CSVPlotDataPreparation:
    ...
    def clear_data(self):
        ...
        CSVPlotDataPreparation.output_scan_index_scaled = np.array([])
        CSVPlotDataPreparation.input_scan_index_scaled = np.array([])
        CSVPlotDataPreparation.list_json_path.clear()
        CSVPlotDataPreparation.mismatch_input_value = np.array([])
        CSVPlotDataPreparation.mismatch_output_value = np.array([])
        CSVPlotDataPreparation.mismatch_input_scan_index = np.array([])
        CSVPlotDataPreparation.mismatch_output_scan_index = np.array([])
    ...
    def consume_event(self, sensor, event):
        ...
        if event == "PDS_UPDATED":
            ...
            for sig in ['ran', 'vel', 'scan_index', 'theta', 'phi']:
                if sig != 'scan_index':
                    ...
                    self._data_event_mediator_obj.notify_event(sensor + "#" + str("DOWNSELECTION") + "#" + sig,
    ...
    def compute_scaled_scan_index(self, sensor, sig):
        ...
        CSVPlotDataPreparation.input_scan_index_scaled = np.repeat(input_scan_index,
                                                                   input_scan_index_duplicate_factor)

        self._plot_datastore_obj.update_data(sig, CSVPlotDataPreparation.input_scan_index_scaled, "mismatch_in",
                                             None,
                                             sensor)
        ...
        CSVPlotDataPreparation.output_scan_index_scaled = np.repeat(output_scan_index,
                                                                    output_scan_index_duplicate_factor)

        self._plot_datastore_obj.update_data(sig, CSVPlotDataPreparation.output_scan_index_scaled, "mismatch_in",
                                             None,
                                             sensor)
    ...
    def compare_vehicle_resim_datapoints(self, sig, sensor):
        ...
        if vehicle_data is not None and resim_data is not None:
            ...
            input_scan_index = self._plot_datastore_obj.get_data("scan_index", "in", sensor)
            output_scan_index = self._plot_datastore_obj.get_data("scan_index", "out", sensor)

            # get_data(self, signal, data_source=None, sensor=None):
            tolerance_dict = {
                "ran": 0.1,
                "vel": 0.015,
                "phi": 0.00873,
            ...
            input_positions = input_positions[input_positions < vehicle_data.shape[0]]
            output_positions = output_positions[output_positions < resim_data.shape[0]]
            ...
            if input_scan_index.ndim != 1 or input_scan_index.shape[0] != vehicle_data.shape[0]:
                raise ValueError("input_scan_index must be a 1D array matching vehicle_data row count")
            ...
            # Ensure it's 1D and matches resim_data row count
            if output_scan_index.ndim != 1 or output_scan_index.shape[0] != resim_data.shape[0]:
                raise ValueError("output_scan_index must be a 1D array matching resim_data row count")
