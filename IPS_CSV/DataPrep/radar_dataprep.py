...
class RadarDataPreparation:
    ...
    def flatten_radar_data(self, sensor, sig):
        ...
        if str(sig) == "theta" or str(sig) == "phi" or str(sig) == "std_theta_1" or str(sig) == "std_phi_1":  # converting rad to degree in place
    ...
    def compute_non_zero_sig_value(self, sig):
        ...
        if sig == "f_bistatic" or sig == "f_single_target" or sig == "f_superres_target":
            # print(f"compute_non_zero_sig_value for {sig}")
            input_signal_value = self._radar_datastore.get_data(sig, "in")
            input_nonzero_counts_row_wise = np.sum(input_signal_value == 1, axis=1)
            # print(f"input_nonzero_counts_row_wise of {sig}",input_nonzero_counts_row_wise)
            ...
            # print(f"output_nonzero_counts_row_wise of {sig}", output_nonzero_counts_row_wise)
