...
class DCJsonToHtmlConvertor:
    ...
    def trigger_json_to_html_conversion(self):
        ...
        for sensor in ["DCTracks"]:
            json_list = self._jsonpath_datastore_obj.get_data(sensor, "scatter")
            json_list_filtered_tracker = self.filter_strings_with_substring("Tracker_Information", json_list)

            if self.is_substring_match_in_longstring("Tracker_Information", json_list_filtered_tracker):
                self.generate_plotly_html_report_new4(json_list_filtered_tracker, sensor + "_tracker_scatter.html")

            json_list = self._jsonpath_datastore_obj.get_data(sensor, "scatter")
            json_list_filtered_osi_grouthtruth = self.filter_strings_with_substring("OSI_Ground_Truth", json_list)

            if self.is_substring_match_in_longstring("OSI_Ground_Truth", json_list_filtered_osi_grouthtruth):
                self.generate_plotly_html_report_new4(json_list_filtered_osi_grouthtruth,
                                                      sensor + "_osigroundtruth_scatter.html")

        for sensor in ["FL", "FR", "RR", "RL"]:
            json_list = self._jsonpath_datastore_obj.get_data(sensor, "scatter")
            json_list_filtered_detection = self.filter_strings_with_substring("Raw_Detection_Information", json_list)

            if self.is_substring_match_in_longstring("Raw_Detection_Information", json_list_filtered_detection):
