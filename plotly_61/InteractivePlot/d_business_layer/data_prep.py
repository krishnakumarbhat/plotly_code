import importlib
from InteractivePlot.c_data_storage.config_storage import Gen7V1_V2, sequence_of_plot
        ...
        for category, plot_keys in sequence_of_plot.items():
            for plot_key in plot_keys:
                if plot_key in signal_plot_data:
                    json_path = signal_plot_data[plot_key]
                    try:
                        with open(json_path, 'r') as f:
                            fig_json = json.load(f)
                        fig = go.Figure(fig_json)
                        key = f"{self.stream_name}_{category}"
                        plots_hash.setdefault(key, []).append(fig)
                        handled_plots.add(plot_key)
                    except Exception as e:
                        logging.error(f"Failed to load plot from {json_path}: {e}")

        # Add any plots not in the sequence (to ensure we don't miss any)
        for plot_key, json_path in signal_plot_data.items():
            if plot_key not in handled_plots:
                try:
                    with open(json_path, 'r') as f:
                        fig_json = json.load(f)
                    fig = go.Figure(fig_json)
                    plots_hash.setdefault(self.stream_name, []).append(fig)
                except Exception as e:
                    logging.error(f"Failed to load plot from {json_path}: {e}")
