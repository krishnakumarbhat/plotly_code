#ifndef ALL_OBJECT_LOG_T_H
#define ALL_OBJECT_LOG_T_H
/*===========================================================================*\
 * FILE: AllObjectsLog.h
 *===========================================================================
 * Copyright 2024  Technologies, Inc., All Rights Reserved.
 * Aptiv Confidential
 *---------------------------------------------------------------------------
 *
 * DESCRIPTION:
...
\*===========================================================================*/
#define ALL_OBJECTS_LOG_STREAM_NUM     5
#define ALL_OBJECTS_LOG_STREAM_VERSION 20
#define ALL_OBJECTS_LOG_NUM_CHUNKS     2

#ifndef __TASKING__
   #pragma pack(push, save_pack_obj, 4)
#else
#endif
typedef struct Object_Log_Extended_Tag
{
   ...
   float vcs_xvel;     //!< [m/s] over-the-ground velocity of object track's reference point along x direction (in VCS coordinate).
   float vcs_yvel;     //!< [m/s] over-the-ground velocity of object track's reference point along y direction (in VCS coordinate).
   ...
   float vcs_heading;  //!< [rad] heading angle of object track's reference point in VCS coordinate.
   float vcs_pointing; //!< [rad] orientation of track's bounding box in VCS coordinate.

   float speed; //!< [m/s, (-10)-100] signed speed of object track's reference point. Negative value indicates track backing up.
   float curvature;  //!< [rad/m] trajectory curvature of object track's reference point in world coordinate.
   ...
   uint8_t current_msmt_type; //!< [0-1] (enum F360_CURRENT_MSMT_TYPE) current measurement type, intermediate unification of
                              //!< interfaces, to be removed after DEX-1436.

   uint8_t padding[2];

} Object_Log_Extended_T;

typedef struct Objects_Log_Tag
{
   Object_Log_Extended_T obj[MAX_F360_OBJECTS];
...
// LOGSIZE_ASSERT(sizeof(All_Objects_Log_T) == (45012));
// LOGSIZE_ASSERT((sizeof(All_Objects_Log_T) / ALL_OBJECTS_LOG_NUM_CHUNKS) * ALL_OBJECTS_LOG_NUM_CHUNKS ==
// sizeof(All_Objects_Log_T));
