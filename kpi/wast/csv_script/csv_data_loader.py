...
class RNumberCSVStrategy(CSVFileStrategy):
    ...
    def identify_files(self, directory: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        if len(csv_files) < 2:
            print(f"Not enough CSV files found in {directory}. Need at least 2 files.")
            return None, None
        ...
        for file in csv_files:
            if re.search(r'_r\d+', file):
                output_candidates.append(os.path.join(directory, file))
            else:
                input_candidates.append(os.path.join(directory, file))
        
        # Check if we have at least one input and one output candidate
        if not input_candidates or not output_candidates:
            print("Could not identify input and output files based on pattern.")
            print(f"Found {len(csv_files)} CSV files: {csv_files}")
            return None, None
        
        # Use the first candidates as input and output
