    ...
    def match_tracks(row):
        ...
        for i in veh_indices:
            ...
            for j in sim_indices:
                ...
                sim_track_f_moving = row.get(f"f_moving_{i}_sim")
...
def find_data_files(base_path):
    ...
    files_of_interest = [s for s in files if '_UDP_GEN7_ROT_OBJECT_STREAM.csv' in s]
    unique_files = set(s.replace('_UDP_GEN7_ROT_OBJECT_STREAM.csv', "").strip() for s in files_of_interest)
    for filename in sorted(unique_files):
        trk_file_path = os.path.join(base_path, filename+'_UDP_GEN7_ROT_OBJECT_STREAM.csv')
        if "_r0" in filename:
...
def process_logs(data_files):
    ...
    for i in range(num_of_logs):
        ...
        if data_files['input']:
            ...
            log_name_wo_ext = file_name.replace("_UDP_GEN7_ROT_OBJECT_STREAM.csv", "")
        html_content += f"""
...
#####################################################################
if len(sys.argv) != 4:
    print("Usage: python tracker_matching_kpi_script.py log_path.txt meta_data.txt output_path")
    sys.exit(1)
