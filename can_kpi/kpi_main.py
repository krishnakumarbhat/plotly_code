...
class KpiMain:
    ...
    def run(self, config_path: str = "kpi.json", output_dir: str | Path = ".") -> Path:
        ...
        for i in range(n_logs):
            ...
            stem = Path(in_path).stem if in_path else (Path(out_path).stem if out_path else f"log_{i+1}")
...
def _parse_args(argv: list[str]) -> tuple[str, Path]:
    ...
    config = _resolve_config_path(argv[1] if len(argv) > 1 and argv[1].lower().endswith(".json") else None)
    out_dir = Path(".")
