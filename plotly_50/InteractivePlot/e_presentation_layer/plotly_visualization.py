...
class PlotlyCharts:
    ...
    def scatter_with_scanindex(data_records, signal_name, sensor_position=""):
        ...
        categories = {
            'input': {'data': data_records['I']+data_records['M'], 'name': 'Input', 'color': 'blue', 'symbol': 'circle'},
            'output': {'data': data_records['O']+data_records['M'], 'name': 'Output', 'color': 'yellow', 'symbol': 'square'},
            ...
            if categories[key]['data']:
                fig.add_trace(go.Scattergl(
                    ...
                    marker=dict(
                        ...
                        size=8 if key == 'mismatched' else 6,
                        opacity=0.7
                    )
                ))
