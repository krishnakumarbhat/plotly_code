...
class DataFetcher:
    ...
    @staticmethod
    def read_hdf_paths(file_path):
        """Read HDF5 file paths from logs.txt."""
        try:
            with open(file_path, 'r') as f:
                return [line.strip().strip('"') for line in f.readlines()]
        except Exception as e:
            print(f"Error reading HDF paths: {e}")
            return []

    @staticmethod
    def process_hdf_file(hdf_path):
        ...
        #now Reading data from the HDF5 file
...
class PlotGenerator:
    ...
    def __init__(self, hdf_path):
        self.processed_data = DataFetcher.process_hdf_file(hdf_path)
        if Application.debug_mode:
            from test_web import Timer
    ...
    def create_plots(self):
        ...
        for i, key in enumerate(self.processed_data.keys()):
            ...
            for k in range(len(plot_t)):
                ...
                # using webgl
                # plot_types[key][f'scattergl_{k}'] = go.Figure(data=[go.Scattergl(
                #             x=self.processed_data[key][f'{plot_t[k]}_{i}'],
                #             y=self.processed_data[key][f'{plot_t[(k + 1) % 3]}_{i}'],
                #             mode='markers',
                #             marker=dict(size=5, line=dict(width=1, color='DarkSlateGrey')),
                #             name=f'Scattergl Plot of {key} {i}'
                #         )])
...
class Application:
    ...
    def __init__(self):
        ...
        hdf_paths = DataFetcher.read_hdf_paths('logs.txt')
        
        for hdf_path in hdf_paths:
            hdf_name = os.path.splitext(os.path.basename(hdf_path))[0]
            self.plot_generator = PlotGenerator(hdf_path)
            self.run(hdf_name)
            
        if self.debug_mode:
            Timer.stop('Application Initialization')
    def run(self, hdf_name):
        ...
        output_filename = os.path.join(output_directory, f'plot_{hdf_name}.html')
...
if __name__ == "__main__":
    ...
    if app.debug_mode:
        ...
        if len(html_files) >= 2:
            ...
            last_html_file_absolute_path = os.path.abspath(last_html_file_relative_path)  # Converting to absolute path
