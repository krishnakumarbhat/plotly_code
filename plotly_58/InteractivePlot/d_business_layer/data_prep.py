...
class DataPrep:
    ...
    def __init__(self, input_data, output_data, html_name,sensor,stream_name, output_dir=None,generate_html=True):
        ...
        logging.debug(f"Input data object size: {get_size_mb(input_data):.2f} MB")
        logging.debug(f"Output data object size: {get_size_mb(input_data):.2f} MB")
        logging.debug(f"Input data container estimated size: {get_container_size_mb(input_data):.2f} MB")
        logging.debug(f"Output data container estimated size: {get_container_size_mb(output_data):.2f} MB")
        logging.debug(f"{'='*50}")
        ...
        b = time.time()
        logging.info(f"Time taken by generate plot figures for {b-a} in {self.stream_name}")

        logging.info(f"\n{'='*50}\nMEMORY USAGE ANALYSIS - {stream_name}\n{'='*50}")
        logging.info(f"plots_hash data object size: {get_size_mb(self.plots_hash):.2f} MB")
        logging.info(f"plots_hash data container estimated size: {get_container_size_mb(self.plots_hash):.2f} MB")
        logging.info(f"{'='*50}")
    ...
    def generate_plots(self):
        ...
        signal_plot_data = {}
        if len(mp_args) > 4 and num_processors > 3:
            try:
