...
class PreRun:
    ...
    def __init__(self, input_file_addr, output_file_addr):
        """
        Initialize the PreRun instance.

        Args:
            input_file_addr (str): Path to the input HDF5 file.
            output_file_addr (str): Path to the output HDF5 file.
        """
        self.input_file_addr = input_file_addr
        self.output_file_addr = output_file_addr
    ...
    def get_missing_data(self):
        ...
        """
        level1_groups_in = [] # this must be sensor
        level2_groups_in = []
        level1_groups_out = [] # this will be strems
        level2_groups_out = []
        ...
        traverse_hdf(self.input_file_addr, self.datasetsin, self.grp_listin, level1_groups_in, level2_groups_in)
        traverse_hdf(self.output_file_addr, self.datasetsout, self.grp_listout, level1_groups_out, level2_groups_out)
...
class PreRunPaired:
    ...
    def __init__(self, input_file_addrs, output_file_addrs):
        """
        Initialize the PreRunPaired instance.

        Args:
            input_file_addrs (list): List of paths to input HDF5 files
            output_file_addrs (list): List of paths to output HDF5 files
        """
        self.input_file_addrs = input_file_addrs
        self.output_file_addrs = output_file_addrs
        
        # Check that input and output files have the same length
        if len(input_file_addrs) != len(output_file_addrs):
    ...
    def analyze_files(self):
        ...
        for i, (input_file_addr, output_file_addr) in enumerate(zip(self.input_file_addrs, self.output_file_addrs)):
            # Initialize data structure for this pair
            self.pair_data[i] = {
                'sensors': [],
                'streams': {},
                'input_file_addr': input_file_addr,
                'output_file_addr': output_file_addr
            }
            
            # Get structure from input and output files
            input_groups, input_streams, input_datasets = self.get_groups_and_datasets(input_file_addr)
            output_groups, output_streams, output_datasets = self.get_groups_and_datasets(output_file_addr)
            ...
            print(f"  Input: {os.path.basename(input_file_addr)}")
            print(f"  Output: {os.path.basename(output_file_addr)}")
