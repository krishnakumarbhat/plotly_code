...
from InteractivePlot.c_data_storage.regex_storage import Gen7V1_v2, Gen7V1_v2_streams
...
class HDF5Parser:
    ...
    @classmethod
    def parse(cls, group: h5py.Group, storage: DataModelStorage, scan_index: List[int], header_names) -> DataModelStorage:
        """
    ...
    def parse_grp(self, group: h5py.Group, header_names) -> None:
        ...
        current_depth = len(group.name.split('/')) - 1  # Root is depth 0, first level is 1, etc.

        # Skip unnecessary processing
        is_second_layer = (current_depth == 2)
        if is_second_layer and current_group_name not in Gen7V1_v2_streams:
        ...
        for item_name, item in group.items():
            if isinstance(item, h5py.Dataset) and item_name in Gen7V1_v2:
