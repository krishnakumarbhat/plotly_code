...
class PairedHdfProcessor:
    ...
    def _process_file(self, hdf_file, sensor: str, stream: str, data_storage: DataModelStorage) -> bool:
        """
        ...
        if sensor_stream_path in hdf_file:
            ...
            if header_path:
                ...
                data_storage = hdf_parser.HDF5Parser.parse(data_group, data_storage, scan_index,header_variants)
                return True
            else:
    ...
    def parse(self):
        ...
        for pair_index, pair_data in enumerate(self.paired_files_list):
            ...
            try:
                            ...
                            for stream in pair_info[i]['streams'][sensor]:
                                ...
                                if input_success or output_success:
                                    try:
                                        ...
                                        logging.debug(f"  Successfully created DataPrep for {sensor}/{stream} in pair {i+1}")
                                    except Exception as e:
                                        print(f"  Error creating DataPrep for {sensor}/{stream} in pair {i+1}: {str(e)}")
                ...
                print(f"Completed processing all sensors and streams of {self.output_dir}")
