...
class HtmlGenerator:
    ...
    def generate_html_content(self):
        ...
        # Generate tabs HTML for navigation - with improved empty check
        tabs_html = ''
        for key in all_plots.keys():
            # Determine if this tab has any plots
            has_plots = False
            if all_plots[key]:
                if isinstance(all_plots[key], dict) and key in all_plots[key]:
                    has_plots = len(all_plots[key][key]) > 0
                else:
                    has_plots = len(all_plots[key]) > 0
            
            # Only add tab if it has plots
            if has_plots:
                tabs_html += f'<div class="tab" onclick="showTab(\'{key.lower().replace(" ", "-")}\')">{key}</div>'

        # Generate content HTML with plots arranged in a grid layout (2 columns) - with improved empty check
        content_html = ''
        for key in all_plots.keys():
            # Determine if this tab has any plots
            has_plots = False
            ...
            if all_plots[key]:
                if isinstance(all_plots[key], dict) and key in all_plots[key]:
                    plots_to_use = all_plots[key][key]
                else:
                    plots_to_use = all_plots[key]
                
                has_plots = len(plots_to_use) > 0
            
            # Only add content if tab has plots
            if has_plots:
                tab_content = f'<div id="{key.lower().replace(" ", "-")}" class="tab-content"><table style="width:100%;">'
                
                for idx in range(0, len(plots_to_use), 2):
                    tab_content += '<tr>'
                    for plot in plots_to_use[idx:idx+2]:
                        tab_content += f'<td style="width:50%;">{plot.to_html(full_html=False, include_plotlyjs=False)}</td>'
                    tab_content += '</tr>'
                
                tab_content += '</table></div>'
                content_html += tab_content
    ...
    def save_html(filename, html_content, output_directory="html"):
        ...
        3. Print a confirmation message with file size and line count
        """
