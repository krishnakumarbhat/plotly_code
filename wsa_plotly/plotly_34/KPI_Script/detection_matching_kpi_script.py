...
#         print(f"Vehicle pairs: {veh_pairs}, Simulation pairs: {sim_pairs}")  # Debugging line
#             required_columns = ['rdd1_num_detect_veh', 'rdd1_num_detect_sim'] + indx_cols_veh + indx_cols_sim
...
def find_data_files(base_path):
    ...
    files_of_interest = [s for s in files if '_UDP_GEN7_DET_CORE.csv' in s or '_UDP_GEN7_RDD_CORE.csv' in s]
    unique_files = set(s.replace('_UDP_GEN7_DET_CORE.csv', "").replace('_UDP_GEN7_RDD_CORE.csv', "").strip() for s in files_of_interest)
    for filename in sorted(unique_files):
        det_file_path = os.path.join(base_path, filename+'_UDP_GEN7_DET_CORE.csv')
        rdd_file_path = os.path.join(base_path, filename+'_UDP_GEN7_RDD_CORE.csv')
        if "_r0" in filename:
...
def process_logs(data_files):
    ...
    for i in range(num_of_logs):
        ...
        if data_files['input']:
            ...
            log_name_wo_ext = file_name.replace("_UDP_GEN7_DET_CORE.csv", "")
        html_content += f"""
