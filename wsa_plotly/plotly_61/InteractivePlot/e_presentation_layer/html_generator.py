...
class HtmlGenerator:
    ...
    def __init__(
        ...
        Parameters:
            signal_plot_paths (dict): A dictionary where keys are plot identifiers and values are paths to individual plot JSONs.
        ...
        self._load_plots_from_json()
        self.generate_and_save_html_files()

    def _load_plots_from_json(self):
        """
        Loads plot data from the signal_plot_paths dictionary.
        """
        if not self.signal_plot_paths:
            logging.warning("No signal plot paths provided to HtmlGenerator.")
            return

        for plot_key, plot_path in self.signal_plot_paths.items():
            if not os.path.exists(plot_path):
                logging.error(f"Plot JSON file does not exist: {plot_path}")
                continue
            try:
                with open(plot_path, 'r') as f_plot:
                    fig_json = json.load(f_plot)
                
                category = 'General'
                if 'kpi' in plot_key.lower():
                    category = 'KPI'
                elif 'signal' in plot_key.lower():
                    category = 'Signal'
                
                self.plots.setdefault(category, []).append(fig_json)
            except Exception as e:
                logging.error(f"Failed to load individual plot from {plot_path}: {e}")
