...
def _load_hyperlink_log_viewer_app():
    module_name = 'hyperlink_html_online_main'
    module = sys.modules.get(module_name)
    if module is None:
        module_path = Path(_hyperlink_tool_base()) / 'main.py'
        if not module_path.exists():
            raise ImportError(f'Hyperlink viewer entrypoint not found at {module_path}')
        spec = importlib.util.spec_from_file_location(module_name, module_path)
        if spec is None or spec.loader is None:
            raise ImportError(f'Could not load Hyperlink viewer module from {module_path}')
...
RUNTIME_TOOL_DEFAULTS = {
    'can_kpi': {'memory': '32G', 'cpus': 8, 'time_limit': '02:00:00', 'partition': 'plcyf-com'},
    'udp_kpi': {'memory': '32G', 'cpus': 8, 'time_limit': '02:00:00', 'partition': 'plcyf-com'},
    'interactive_plot': {'memory': '64G', 'cpus': 8, 'time_limit': '04:00:00', 'partition': 'plcyf-com'},
    'rag': {'memory': '48G', 'cpus': 8, 'time_limit': '08:00:00', 'partition': 'plcyf-com'},
}
...
def _sync_runtime_job(job: JobHistory):
    ...
    job.output_log_path = runtime_job.get('log_path') or job.output_log_path
...
def _submit_runtime_job(tool_key: str, mode: str, paths: dict, resources: dict, job_tool_name: str, input_path: str):
    payload = {
        'tool_key': tool_key,
        'variant': tool_key,
        'mode': mode,
    ...
    db.session.add(job)
    db.session.commit()
    flash(f'{tool_key} request queued through HPCC broker. Runtime job: {result["job_id"]}', 'success')
    return redirect(url_for('dashboard'))


def submit_runtime_kpi_job():
    execution_target = (request.form.get('execution_target') or 'udp_kpi').strip()
    input_mode = (request.form.get('input_mode') or 'json').strip().lower()
    default_resources = _default_runtime_resources(execution_target)
    resources = {
        'scheduler': (request.form.get('scheduler') or 'slurm').strip().lower(),
    ...
    if execution_target == 'interactive_plot':
        ...
        if input_mode == 'json' and (not config_xml or not json_path):
            flash('Interactive plot requires both XML and JSON inputs.', 'error')
            return redirect(request.referrer or url_for('tool_kpi'))
        if input_mode == 'hdf' and (not input_hdf or not output_hdf):
            flash('Interactive plot HDF mode requires input and output HDF paths.', 'error')
            return redirect(request.referrer or url_for('tool_kpi'))
        paths.update({
        ...
        if input_mode == 'json' and not json_path:
            flash('JSON path is required for KPI JSON mode.', 'error')
            return redirect(request.referrer or url_for('tool_kpi'))
        if input_mode == 'hdf' and (not input_hdf or not output_hdf):
            flash('Input and output HDF paths are required for KPI HDF mode.', 'error')
            return redirect(request.referrer or url_for('tool_kpi'))
        paths.update({
            'json_path': json_path,
            'input_hdf': input_hdf,
    ...
    try:
        return _submit_runtime_job(execution_target, input_mode, paths, resources, 'kpi', primary_input)
    except Exception as exc:
...
def chat_api():
    ...
    try:
        ...
        if rag_response.get('ok'):
            ...
            response = (
                'RAG service is not reachable right now. '
                'You can still use KPI and Hyperlink tools while the runtime map is being configured.\n\n'
...
def api_runtime_tools():
    ...
    graph_payload = runtime_store.graph_payload()
    try:
...
def api_runtime_launch():
    ...
    job = JobHistory(
        ...
        output_path=result.get('output_path', ''),
        output_log_path=result.get('log_path', ''),
        parameters={'runtime_job_id': result['job_id'], 'execution_target': tool_key, 'paths': paths, 'resources': resources},
        status='QUEUED',
    )
    db.session.add(job)
    db.session.commit()
