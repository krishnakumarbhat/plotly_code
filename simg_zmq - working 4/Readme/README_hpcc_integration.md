...
1. The user opens `main_html` in the browser.
2. KPI requests are submitted through the UI.
3. `main_html` sends runtime requests to `hpcc_main.py` over a local JSON-over-socket protocol.
4. `hpcc_main.py` records the request, chooses the correct image, launches the job, and updates runtime status in SQLite.
5. RAG is called over HTTP and remains decoupled as a service.
6. Hyperlink is integrated directly into `main_html` and reuses the same session context.
...
`rag.simg` is a standalone HTTP service used by the dashboard chat.
...
| `simg_sh_hpcc/rag.simg` | RAG service container |
| `simg_sh_hpcc/can_kpi.simg` | CAN KPI image |
| `simg_sh_hpcc/udp_kpi.simg` | UDP KPI image |
| `simg_sh_hpcc/interactiveplot.simg` | Interactive Plot image |
...
|------|----------|-------|-------------|
| `5001` | HTTP | `main_html` | main browser UI |
| `5100` | HTTP | `rag` | RAG service |
| `9100` | TCP socket | `hpcc_main.py` | broker API |
...
3. `rag_client` calls `rag.simg` over HTTP.
