...
class KpiDataModel:
    def __init__(self, data_container_in, val_sig_map_in, sig_val_map_in, data_container_out, val_sig_map_out,sig_val_map_out):
        self.create_html( data_container_in, val_sig_map_in, sig_val_map_in, data_container_out,val_sig_map_out, sig_val_map_out)
    def create_html(self, data_container_in, val_sig_map_in, sig_val_map_in, data_container_out,val_sig_map_out, sig_val_map_out):
        """
        Generate an HTML string that includes the input and output data.
        
        Parameters:
            data_container_in: dict-like structure containing input data.
            val_sig_map_in: mapping of values to signatures for input data.
            sig_val_map_in: mapping of signatures to values for input data.
            data_container_out: dict-like structure containing output data.
            sig_val_map_out: mapping of signatures to values for output data.
        
        Returns:
            A string containing the HTML representation of the data.
        """
        ...
        # Create a section for input data
        html.append('    <h2>Input Data</h2>')
        html.append('    <table>')
        html.append('        <tr><th>Key</th><th>Value</th><th>Signature</th></tr>')
        for key, value in data_container_in.items():
            # Retrieve signature via mapping if available
            # signature = val_sig_map_in.get(value, "N/A")
            # Alternatively, if you use sig_val_map_in, you can lookup value from signature.
            # For demonstration we use the value from val_sig_map_in.
            html.append(f'        <tr><td>{key}</td><td>{value}</td><td></td></tr>')
        html.append('    </table>')

        # Create a section for output data
        html.append('    <h2>Output Data</h2>')
        html.append('    <table>')
        html.append('        <tr><th>Signature</th><th>Value</th></tr>')
        for sig, val in data_container_out.items():
            # Use sig_val_map_out if needed
            mapped_value = sig_val_map_out.get(sig, "N/A")
            html.append(f'        <tr><td>{sig}</td><td>{val} | Mapped: {mapped_value}</td></tr>')
        html.append('    </table>')
        ...
        html_output = "\n".join(html)
        # print(html_output)  # Print the HTML to stdout
