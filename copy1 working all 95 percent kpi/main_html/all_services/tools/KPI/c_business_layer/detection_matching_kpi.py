...
class DetectionMappingKPIHDF:
    ...
    def process_rdd_matching(self):
        ...
        # NOTE: "Matching Accuracy" is defined as overlap of DETECTION_STREAM `rdd_idx`
        # between vehicle and simulation, using multiset intersection (handles duplicates).
        total_matches = 0
        total_detections = 0
        per_scan_accuracy = []
        ...
        # Total detections for accuracy is computed as the number of UNIQUE `rdd_idx`
        # per scan (summed across scans). This aligns with how matching is interpreted:
        # "does this detection index exist on both sides".
        ...
        def _set_from_row(row, count: int) -> set[int]:
            out: set[int] = set()
            if row is None:
                return out
            try:
                limit = min(count, len(row))
            except Exception:
                limit = 0
            for k in range(limit):
                try:
                    rid = row[k]
                    if isinstance(rid, float) and np.isnan(rid):
                        continue
                    out.add(int(rid))
                except Exception:
                    continue
        ...
        for i, arr in enumerate(veh_rdd.get('num_af_det', [])):
            ...
            if i < len(sim_rdd.get('num_af_det', [])):
                sim_den = _as_int_count(sim_rdd['num_af_det'][i])

            # UNIQUE set overlap on `rdd_idx` drives matches.
            veh_set = _set_from_row(veh_det_rdd_idx[i] if i < len(veh_det_rdd_idx) else None, den)
            sim_set = _set_from_row(sim_det_rdd_idx[i] if i < len(sim_det_rdd_idx) else None, sim_den)

            matches = 0
            try:
                matches = len(veh_set & sim_set)
            except Exception:
                matches = 0

            den_unique = len(veh_set)
            total_detections += den_unique

            # Debug rdd_idx overlap for first few scans
            if i < 5:
                logger.info(
                    "Scan %s: rdd_idx overlap=%s / veh=%s sim=%s",
                    i,
                    matches,
                    len(veh_set),
                    len(sim_set),
            ...
            acc_i = (matches / den_unique) * 100.0 if den_unique > 0 else None
            per_scan_accuracy.append(acc_i)
            matched_scan_indices.append(i)
            per_scan_matches.append(int(matches))
            per_scan_den.append(int(den_unique))
