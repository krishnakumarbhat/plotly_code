...
class DataToJSONConvert(ISigObserver):
    ...
    def _scatter(self, sensor, stream, sig, unit):
        ...
        if in_data is not None:
            ...
            fig.add_trace(go.Scattergl(x=in_idx.tolist() if in_idx is not None else None, y=in_data.tolist(), mode='markers', marker=dict(size=3, opacity=0.5, color="blue"), name="input"))
        if out_data is not None:
            out_idx = self._plot_ds.get_data(sig, "out_scan_index", sensor)
            fig.add_trace(go.Scattergl(x=out_idx.tolist() if out_idx is not None else None, y=out_data.tolist(), mode='markers', marker=dict(size=3, opacity=0.5, color="red"), name="output"))
    ...
    def _mismatch_scatter(self, sensor, stream, sig, unit):
        ...
        fig = go.Figure([go.Scattergl(x=idx, y=data, mode='markers', marker=dict(size=3, opacity=0.5, color="blue"), name='input')])
        fig.update_layout(xaxis_title="Scan Index (ms)", yaxis_title=str(unit))
