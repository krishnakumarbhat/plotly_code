...
class DistanceToCSOriginBetweenThrFlag(IFlag):
    """ Class for configurable flag calculation: check if targets' distance to coordinate system origin
        is in between two given thresholds """

    def __init__(self, pos_x_signal_name='center_x', pos_y_signal_name='center_y', thr_low=np.inf, thr_high=np.inf,
                 *args, **kwargs):
        """
    ...
    def calc_flag(self, data_frame: pd.DataFrame, *args, **kwargs) -> pd.Series:
        ...
        f_valid = (distance <= self.thr_high) & (distance >= self.thr_low)
