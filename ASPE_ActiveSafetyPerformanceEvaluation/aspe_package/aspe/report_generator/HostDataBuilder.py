import numpy as np
import pandas as pd
from aspe.report_generator.BaseBuilder import BaseBuilder
from aspe.report_generator.Plotter import Plotter
from aspe.report_generator.support_functions import calc_cdf
from aspe.report_generator.support_functions import load_pickle


class HostDataBuilder(BaseBuilder):
    def __init__(self, host_data, template_name, plotter):
        """
        Class for building host data report section. Contains information about speed and yaw rate.
        :param host_data: dict: soft_version: DataFrame with host data - output from PE
        :param template_name: string, .html template file name
        :param plotter: object, report_generator.Plotter class instance
        """
        BaseBuilder.__init__(self, template_name, plotter)
        self.host_data = host_data
        self.init_template_args()
        self.build_results()
        self.render_html()

    def init_template_args(self):
        self.template_args['velocity_plots'] = []
        self.template_args['yaw_rate_plots'] = []

    def build_results(self):
        """"Velocity information"""
        vel_cdf, min_val, max_val = self.calc_cdf(np.abs(self.host_data.loc[:, 'raw_speed']))
        vel_pdf_plot_path = self.plotter.histogram(self.host_data.loc[:, 'raw_speed'], 'Host speed histogram',
                                                   xlabel='Speed [m/s]', ylabel='Count [-]', figsize=(10, 3))
        vel_cdf_plot_path = self.plotter.cdf(vel_cdf, [], 'Host speed empirical CDF', xlabel='Speed [m/s]',
                                             ylabel='Count []', figsize=(10, 3))

        """"Yaw rate information"""
        yaw_rate_cdf, min_val, max_val = self.calc_cdf(np.abs(np.rad2deg(self.host_data.loc[:, 'yaw_rate'])))
        yaw_pdf_plot_path = self.plotter.histogram(self.host_data.loc[:, 'yaw_rate'], 'Host yaw rate histogram',
                                                   xlabel='Yaw rate [deg/s]', ylabel='Count [-]',
                                                   figsize=(10, 3))
        yaw_cdf_plot_path = self.plotter.cdf(yaw_rate_cdf, [], 'Host yaw rate empirical CDF', xlabel='Absolute yaw rate [deg/s]',
                                             figsize=(10, 3))

        self.template_args['velocity_plots'].append(vel_pdf_plot_path)
        self.template_args['velocity_plots'].append(vel_cdf_plot_path)
        self.template_args['yaw_rate_plots'].append(yaw_pdf_plot_path)
        self.template_args['yaw_rate_plots'].append(yaw_cdf_plot_path)

    def calc_cdf(self, data):
        n = 1000
        column_data = data[~np.isnan(data)]  # delete nan from vector
        min_val = column_data.min()
        max_val = column_data.max()
        x = np.linspace(min_val, max_val, n)
        y_cdf = calc_cdf(x, column_data)

        cdf_df = pd.DataFrame({'CDF': y_cdf}, index=x)
        return cdf_df, min_val, max_val


if __name__ == "__main__":
    host_data_path = r"\\10.224.186.68\AD-Shared\F360\Logs\AIT-646_RNA_Basic_KPI_report\2_FTP\full_loop\SW201_SUV_V1\LSS\pe_example\pe_example_agg_host_data"
    config = {'plot_style': 'default',
              'plot_size': (10, 4),
              'figures_save_path': r"\\10.224.186.68\AD-Shared\F360\Logs\AIT-646_RNA_Basic_KPI_report\2_FTP\full_loop\SW201_SUV_V1\LSS\pe_example\report_module\temp_plots"}
    host_data = load_pickle(host_data_path)
    template_path = "HostData.html"
    plotter = Plotter(config)
    host_data_builder = HostDataBuilder(host_data, template_path, plotter)
    host_data_builder.save_pdf(r'\\10.224.186.68\AD-Shared\F360\Logs\AIT-646_RNA_Basic_KPI_report\2_FTP\full_loop\SW201_SUV_V1\LSS\pe_example\report_module\pdf_reports\host_data.pdf')
