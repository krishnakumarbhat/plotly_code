...
class CTAScenariosEventFinder(BaseNexusEventFinder):
    def __init__(self, config: Nexus50kEvaluationConfig):
        super().__init__(config, 'cta_scenarios')
    ...
    def find_event(self, extracted_data: NexusExtractedData) -> Tuple[float, float]:
        ...
        if len(host_filtered) > 0:
            ...
            cta_objects = cta_objects.query(f'{time_min} < timestamp & timestamp < {time_max}')

            ids_correct = []
            for id, item in cta_objects.groupby('unique_id'):
                if (item.position_y.min() < highest_allowed_min_position_y) and (
                        item.position_y.max() > lowest_allowed_max_position_y) and (
                        item.speed.max() > lowest_allowed_max_object_speed):
