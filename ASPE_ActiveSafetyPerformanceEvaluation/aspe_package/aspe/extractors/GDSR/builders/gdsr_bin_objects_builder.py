...
class GdsrBinObjectsBuilder(IBuilder):
    signal_mapper = {
        # GDSR bin signature: ASPE signature
        "distinct_id": "id",
    ...
    def __init__(self, parsed_data, f_extract_raw_signals=False):
        ...
        self.data_set: GdsrObjects = GdsrObjects()
        self._f_extract_raw_signals = f_extract_raw_signals
        self._raw_signals = pd.DataFrame()

    def build(self) -> GdsrObjects:
        """Extracts objects from parsed data.

        :return: an instance of GdsrObjects with extracted signals.
        """
    ...
    def _create_lifespan_mask(self):
        status_array = self._parsed_data["parsed_data"]["status"].copy()
    ...
    def _extract_raw_signals(self):
        ...
        num_objects = self._parsed_data["parsed_data"]["distinct_id"].shape[-1]
    ...
    def _extract_mappable_signals(self):
        for gdsr_signature, aspe_signature in self.signal_mapper.items():
            if gdsr_signature not in self._raw_signals.columns:
                warn(
            ...
            self.data_set.signals[aspe_signature] = self._raw_signals[
                gdsr_signature
            ].to_numpy()
    ...
    def _extract_classification(self):
        ...
        class_to_class_prob_signal_map = {
            # GDSR class: GDSR class probability signal name
