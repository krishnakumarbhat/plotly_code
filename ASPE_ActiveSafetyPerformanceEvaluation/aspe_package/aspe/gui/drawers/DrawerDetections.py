...
class DrawerDetections(DrawerComposite):
    ...
    def create_selection_drawer(self, plot_item):
        return DrawerPoints(self, "DetectionSelection", plot_item, symbol="o", color="#ff0000", symbol_size=10)

    def create_drawers(self, plot_item):
        detections_points_drawer = DrawerPoints(
            self, "DetectionPoint", plot_item, symbol="x", color="#800080", symbol_size=8)
        range_rates_drawer = DrawerRangeRates(
            self, "RangeRate", plot_item, color="#6464C8", line_style="-", line_width=1)
        label_drawer = DrawerLabels(self, "Labels", plot_item, color="#6464C8")

        detections_points_drawer.set_data(
            self.data_set.signals, "position_x", "position_y")
        range_rates_drawer.set_data(
            self.data_set.signals, "position_x", "position_y", self.rrate_signal_name, self.azimuth_signal_name)
        label_drawer.set_data(
            self.data_set.signals, self.data_set.raw_signals, "position_x", "position_y")

        return [detections_points_drawer, range_rates_drawer, label_drawer]
