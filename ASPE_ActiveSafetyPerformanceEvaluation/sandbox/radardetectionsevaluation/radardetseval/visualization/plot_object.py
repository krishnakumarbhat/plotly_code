...
from AptivDataExtractors.utilities.MathFunctions import lcs2vcs, pol2cart

''' TODO: This is copy paste from aptivperformanceevaluation. should be removed.'''


def plot_object_wrapper(ax, tracked_object: pd.Series, velocity_scale=1.0, **kwargs):
    """
    Wrapper for object ploting
    :param ax: axes handle
    :type ax: matplotlib.axes.Axes
    :param tracked_object: single tracked object
    :type ax: pd.Series
    :param velocity_scale - scale for velocity vectors, default 1.0
    :param kwargs:
    :return:
    """
    plot_object(ax, None, tracked_object.position_x, tracked_object.position_y,
                tracked_object.velocity_otg_x, tracked_object.velocity_otg_y,
                tracked_object.center_x, tracked_object.center_y,
                tracked_object.bounding_box_dimensions_x, tracked_object.bounding_box_dimensions_y,
                tracked_object.bounding_box_orientation, velocity_scale=velocity_scale, **kwargs)
...
def plot_object(ax: matplotlib.axes.Axes, object_id, ref_point_x, ref_point_y, vx, vy, center_x, center_y,
    ...
    plot_bb_orientation(ax, center_x, center_y, length, width, orientation, **kwargs)
...
def plot_bb_ref_point(ax: matplotlib.axes.Axes, x, y, object_id=None, **kwargs):
    ...
    if object_id is not None:
        ax.annotate(str(object_id), (y, x), **kwargs)
