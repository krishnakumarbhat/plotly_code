...
class Simulator:
    ...
    def __call__(self, i=1):
        ...
        detection = self.det_simulator.sim_single_point_from_ref_point(self.meas_cov,
                                                                       self.ref_point[0],
                                                                       self.ref_point[1])
        opt_output = find_nearest_det_on_object_fast(radar_object, detection)
        opt_output['gt_ref_point_x'] = self.ref_point[0]
        opt_output['gt_ref_point_y'] = self.ref_point[1]
        return opt_output


def simulate_object_from_ref_point(gt_radar_object, ref_point, meas_bias: np.array = np.zeros(3),
    ...
    sim = Simulator(gt_radar_object, pos_cov, vel_cov, meas_cov, ref_point, meas_bias)
...
if __name__ == '__main__':
    ...
    gt_ref_point = np.array([0.5, 0.5])
    simulate_object_from_ref_point(gt_object, gt_ref_point, np.zeros(3),
                                   2000, r'private\single_object_x6_y5_vx4_vy0_ref_point_05_05_2000.pickle',
                                   True, 8)
