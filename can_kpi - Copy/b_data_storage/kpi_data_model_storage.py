...
class KpiDataModelStore:
    """Project store wrapper backed by ``KPI_DataModelStorage`` per sensor and label."""
    ...
    def add_parsed_data(self, label: str, parsed: Dict[str, Any]) -> None:
        self._stores[label] = {}
        for sensor_id, sensor_data in parsed.items():
            scan_index = np.asarray(sensor_data.get("scan_index", np.array([]))).astype(
                int
            )
            model = KPI_DataModelStorage()
            model.initialize(scan_index.tolist(), sensor_id)

            detection_signals = sensor_data.get("detection_signals", {})
            n_scans = len(scan_index)
            for signal_name, det_map in detection_signals.items():
                if not det_map:
                    continue
                model.init_parent(signal_name)
                for det_idx in sorted(det_map.keys()):
                    values = np.asarray(det_map[det_idx], dtype=float).ravel()
                    if len(values) < n_scans:
                        padded = np.full(n_scans, np.nan)
                        padded[: len(values)] = values
                        values = padded
                    elif len(values) > n_scans:
                        values = values[:n_scans]
                    model.set_value(
                        values.tolist(),
                        f"{signal_name}_{int(det_idx):03d}",
                        signal_name,
                    )

            self._stores[label][sensor_id] = {
                "friendly_name": sensor_data.get("friendly_name", sensor_id),
                "scan_index": scan_index,
                "n_scans": int(len(scan_index)),
                "max_det_index": int(sensor_data.get("max_det_index", 0)),
                "alignment_signals": sensor_data.get("alignment_signals", {}),
                "header_signals": sensor_data.get("header_signals", {}),
                "model": model,
            }
    ...
    def get_scan_index(self, label: str, sensor_id: str) -> np.ndarray:
        sensor = self.get_sensor(label, sensor_id)
        if sensor is None:
            return np.array([])
        return sensor.get("scan_index", np.array([]))
    ...
    def get_detection_signal(
        ...
        for key in model._signal_to_value.keys():
            ...
            if not suffix.isdigit():
                continue
            det_idx = int(suffix)
            values, status = KPI_DataModelStorage.get_value(model, key, signal_name)
            if status != "success" or values.size == 0:
                continue
            if values.ndim == 1:
                out[det_idx] = values
            else:
                out[det_idx] = values[:, 0]
        return out if out else None

    def get_detection_2d(
        self, label: str, sensor_id: str, signal_name: str
    ) -> Tuple[np.ndarray, np.ndarray]:
        scan_index = self.get_scan_index(label, sensor_id)
        signal_map = self.get_detection_signal(label, sensor_id, signal_name)
        if signal_map is None:
            return scan_index, np.empty((len(scan_index), 0))

        max_det = max(signal_map.keys())
        arr = np.full((len(scan_index), max_det), np.nan)
        for det_idx, values in signal_map.items():
            col = det_idx - 1
            length = min(len(values), len(scan_index))
            arr[:length, col] = values[:length]
        return scan_index, arr
    ...
    def get_alignment_signals(
        ...
        if sensor is None:
            return {}
        return sensor.get("alignment_signals", {})

    def get_header_signals(self, label: str, sensor_id: str) -> Dict[str, np.ndarray]:
        sensor = self.get_sensor(label, sensor_id)
        if sensor is None:
            return {}
        return sensor.get("header_signals", {})
    ...
    def summary(self) -> str:
        ...
        for label, sensors in self._stores.items():
            ...
            for sensor_id, sensor_data in sensors.items():
                lines.append(
                    f"  {sensor_id} ({sensor_data.get('friendly_name')}): "
                    f"{sensor_data.get('n_scans', 0)} scans, "
                    f"{sensor_data.get('max_det_index', 0)} max det"
                )
        return "\n".join(lines)
