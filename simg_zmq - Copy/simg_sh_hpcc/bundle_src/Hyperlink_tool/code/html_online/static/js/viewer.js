...
function renderSavedList() {
  ...
  entries.forEach(function (item) {
    ...
    meta.textContent = entry.video_name || entry.remote_video_path || 'Cached locally';
...
async function fetchVideoBlob(url, onProgress, loadToken) {
  const chunks = [];
...
async function loadVideoIntoPlayer(video, entry, label) {
  ...
  try {
    const blob = await fetchVideoBlob(entry.video, function (downloaded, expectedSize) {
      if (loadToken !== activeVideoLoadToken) {
        return;
      }
      if (expectedSize) {
    ...
    if (loadToken !== activeVideoLoadToken || !blob) {
      return;
    }

    activeVideoBlobUrl = URL.createObjectURL(blob);
    video.src = activeVideoBlobUrl;
    video.preload = 'auto';
    video.load();
...
async function updateScenario(key) {
  ...
  if (!currentKey) {
    ...
    video.removeAttribute('src');
    video.load();
    videoInfo.textContent = 'No video';
    commentBox.value = '';
  ...
  toolbarMeta.textContent = entry.remote_html_path || entry.video_name || 'Cached stream';
...
function displayRemoteLogs() {
  ...
  logKeys.forEach(function (key) {
    ...
    status.textContent = log.saved ? 'Already saved to your stream list' : 'Ready to add';
