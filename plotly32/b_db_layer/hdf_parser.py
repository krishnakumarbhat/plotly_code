...
class HDF5Parser:
    ...
    def __init__(self, group, data_container, val_sig_map, scan_index):
        self.group = group
        self.data_container = data_container
        self.val_sig_map = val_sig_map
        self.val_sig_map_tenth = 0
        self.val_sig_map_unit = 0
        self.grp_stack = []  # Stack for DFS traversal
    ...
    def parse(cls, group, data_container, val_sig_map, scan_index):
        ...
        parser_instance = cls(group, data_container, val_sig_map, scan_index)
            ...
            for row, scanidx in itertools.zip_longest(dataset, self.data_container, fillvalue=[]):
                if row is not None and len(row) > 0:
                    ...
                    if self.val_sig_map_unit == 0:
                        ...
                        self.data_container[scanidx][-1].append(row)
                    print(f"Missing data in {dataset_name} in {group.name}")

            self.val_sig_map_unit += 1

        # Process child groups recursively
        while self.grp_stack:
            current_group = self.grp_stack.pop(0)
