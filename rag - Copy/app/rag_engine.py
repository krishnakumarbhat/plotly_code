...
class RagEngine:
    ...
    def _extract_threshold_from_question(question: str) -> tuple[str, float] | None:
        ...
        if "recall" not in question_lower:
            return None
    ...
    def _answer_recall_filter_query(self, question: str) -> str | None:
        ...
        for node in docs.values():
            ...
            if not self._compare(operator, recall_value, threshold):
                continue
            ...
            entries.append((label, recall_value))

        if not entries:
            return (
                f"No logs in the ingested narratives have recall {operator} {threshold:.4f}. "
                "If you expected matches, re-run ingestion with --reset-index and confirm matched_files > 0."
            )

        entries.sort(key=lambda item: item[1], reverse=True)
        lines = [f"Logs with recall {operator} {threshold:.4f}:", ""]
        for label, recall_value in entries[:80]:
            lines.append(f"- {label}: recall={recall_value:.4f}")
        if len(entries) > 80:
            lines.append("")
            lines.append(f"Showing first 80 of {len(entries)} matches.")
        return "\n".join(lines)
    ...
    def _call_llama_server(self, messages: list[dict[str, str]]) -> str:
        ...
        continuation_content, continuation_finish_reason = self._llama_server_chat_completion(
            messages=continuation_messages,
            max_tokens=max(120, self._llm_max_new_tokens // 2),
        )
        merged = f"{content}\n{continuation_content}".strip()
        if continuation_finish_reason == "length":
            merged += "\n\n[Response truncated. Increase LLM_MAX_NEW_TOKENS for longer answers.]"
        return merged
    ...
    def answer(self, question: str, history: list[dict[str, str]] | None = None) -> str:
        ...
        recall_filter_answer = self._answer_recall_filter_query(question)
        if recall_filter_answer is not None:
            return recall_filter_answer
        ...
        snippets = self._extract_snippets(retrieved_nodes, max_snippets=max_snippets, max_chars=850)
        context_text = "\n\n".join(snippets)[:4200]
